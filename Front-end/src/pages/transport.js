    ...
    if (this.props.cliente.tipo == "Estandar") {
      return (
        <div>
          ...
          >
            <span>
              SU NIVEL DE USUARIO NO TIENE AUTORIZACIÓN AL ACCESO DE ESTE
              SERVICIO
            </span>
            <span style={{ marginTop: "20px" }}>
              AUMENTE SU NIVEL A CLIENTE FRECUENTE PARA DISFRUTAR DEL SERVICIO
            </span>
          </div>
          <Footer />
        </div>
              ...
              {Object.values(this.props.services).map((val, key) =>
                val.nombre === "Transporte" ? (
                  <TarjetaTrans
                    ...
                    serv={val}
                    name={val.tipo === "Avion" ? "Avión" : val.tipo}
                    imagen={val.tipo.toLowerCase().replace(/\s/g, "") + ".jpg"}
                  />
                ) : (
                  <div></div>
                )
