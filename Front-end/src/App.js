...
class App extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (entire_client != null) entire_client = JSON.parse(entire_client);
    this.props.dispatch(loginAction(isLoged, id));
    this.props.dispatch(savetheclient(entire_client));

    const all_serv = await fetch(
      "http://localhost:8080/Concierge/rest/service/",
      {
        method: "GET",
        mode: "cors",
      }
    ...
    if (
      ...
      this.setState({
        primer_componente: (
          <NavbarInicio login={isLoged} id={id} click={this.updateNavBar} />
        ),
      });
    } else {
      this.setState({
        primer_componente: (
          <Navbar login={isLoged} id={id} click={this.updateNavBar} />
        ),
      });
    }
  }
  ...
  loginUpdate() {
    window.location.href =
      "http://localhost:3000" + process.env.PUBLIC_URL + "/";
    sessionStorage.setItem("login", true);
    sessionStorage.setItem("cliente", this.props.id_cliente);
    sessionStorage.setItem(
      "entire_client",
      JSON.stringify(this.props.entire_client)
    );
            ...
            <Switch>
              ...
              <Route path="/leisure">
                <div>
                  ...
                  <Leisure
                    login={this.props.login}
                    services={this.props.all_services}
              ...
              <Route path="/premium">
                <div>
                  ...
                  <OtherService
                    login={this.props.login}
                    id_cliente={this.props.id_cliente}
                    entire_client={this.props.entire_client}
                  />
                </div>
              </Route>
              <Route path="/profile">
                <div>
                  ...
                  <Profile
                    login={this.props.login}
                    savetheuser={(entire_client) => {
                      this.props.dispatch(savetheclient(entire_client));
                    }}
                    entire_client={this.props.entire_client}
                    id_cliente={this.props.id_cliente}
                    logOut={this.logedOut}
                  />
                </div>
              </Route>
